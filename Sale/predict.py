...
def predict(model, dataloader, config, logger=None):
  ...
  for epoch in range(config.EPOCH_NUM):
      ...
      for inputs, labels in bar:
          ...
          inputs = paddle.round(paddle.reshape(inputs, shape=(-1,)))
          outputs = paddle.round(paddle.reshape(outputs, shape=(-1,)))
          print(inputs.tolist())
...
if __name__ == "__main__":
    ...
    # 加载数据集
    dataloader = DataLoader(SaleDataset(1),batch_size=config.BATCH_SIZE,shuffle=config.SHUFFLE,drop_last=config.DROP_LAST)
    # 加载模型
    model = SaleModel(config.HIDDEN_SIZE, num_layers=config.NUM_LAYERS, dropout_rate=config.DROPOUT_RATE)

    # 保存模型，包含两部分：模型参数和优化器参数
    model_name = "sale_regressor_10"
    param_dict = paddle.load("logs/{}.pdparams".format(model_name))
