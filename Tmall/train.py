...
def train(model, dataloader, config, logger=None):
  ...
  for epoch in range(config.EPOCH_NUM):
      ...
      for inputs,labels in bar:
          ...
          bar.postfix = "{}/{}, Loss: {:.6f} Output:{:.0f},{:.0f}".format(epoch+1, config.EPOCH_NUM, loss.numpy()[0], labels.tolist()[0][-config.UNKNOWN_SEQ],outputs.tolist()[0][-config.UNKNOWN_SEQ])
...
if __name__ == "__main__":
    ...
    config.UNKNOWN_SEQ = 3

    # 加载数据集
    dataloader = DataLoader(TmallDataset(config.LEN_SEQ, config.UNKNOWN_SEQ),batch_size=config.BATCH_SIZE,shuffle=config.SHUFFLE,drop_last=config.DROP_LAST)
    # 加载模型
